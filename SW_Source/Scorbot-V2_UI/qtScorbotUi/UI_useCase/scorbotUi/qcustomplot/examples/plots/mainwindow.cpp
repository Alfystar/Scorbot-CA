...
void MainWindow::setupSincScatterDemo (QCustomPlot *customPlot){
    ...
    customPlot->addGraph();
    customPlot->legend->removeItem(
            customPlot->legend->itemCount() - 1); // don't show two confidence band graphs in legend
    ...
    for (int i = 0; i < 250; ++i){
        x0[i] = (i / 249.0 - 0.5) * 30 +
                0.01; // by adding a small offset we make sure not do divide by zero in next code line
...
void MainWindow::setupScatterStyleDemo (QCustomPlot *customPlot){
    ...
    for (int i = 0; i < shapes.size(); ++i){
        ...
        customPlot->graph()->setName(QCPScatterStyle::staticMetaObject.enumerator(
...
void MainWindow::setupMultiAxisDemo (QCustomPlot *customPlot){
    ...
    customPlot->setLocale(QLocale(QLocale::English,
                                  QLocale::UnitedKingdom)); // period as decimal separator and comma as thousand separator
    customPlot->legend->setVisible(true);
...
void MainWindow::setupLogarithmicDemo (QCustomPlot *customPlot){
    ...
    QVector <QCPGraphData> dataLinear(dataCount), dataMinusSinExp(dataCount), dataPlusSinExp(dataCount), dataFactorial(
            dataFactorialCount);
...
void MainWindow::setupParametricCurveDemo (QCustomPlot *customPlot){
    ...
    for (int i = 0; i < pointCount; ++i){
        ...
        dataDeltoid[i] = QCPCurveData(i, 2 * qCos(2 * theta) + qCos(1 * theta) + 2 * qSin(theta),
...
void MainWindow::setupStatisticalDemo (QCustomPlot *customPlot){
    ...
    statistical->addData(2, 0.8, 1.6, 2.2, 3.2, 4.9,
                         QVector <double>() << 0.7 << 0.34 << 0.45 << 6.2 << 5.84); // provide some outliers as QVector
...
void MainWindow::setupItemDemo (QCustomPlot *customPlot){
    ...
    phaseTracerArrow->startDir->setCoords(-40,
                                          0); // direction 30 pixels to the left of parent anchor (tracerArrow->start)
    ...
    phaseTracerArrow->setTail(QCPLineEnding(QCPLineEnding::esBar, (phaseTracerText->bottom->pixelPosition().y() -
                                                                   phaseTracerText->top->pixelPosition().y()) * 0.85));
    ...
    groupTracerArrow->startDir->setCoords(-40,
                                          0); // direction 30 pixels to the left of parent anchor (tracerArrow->start)
    ...
    groupTracerArrow->setTail(QCPLineEnding(QCPLineEnding::esBar, (groupTracerText->bottom->pixelPosition().y() -
                                                                   groupTracerText->top->pixelPosition().y()) * 0.85));
...
void MainWindow::setupStyledDemo (QCustomPlot *customPlot){
    ...
    for (int i = 0; i < x3.size(); ++i){
        ...
        y3[i] = 0.05 + 3 * (0.5 + qCos(x3[i] * x3[i] * 0.2 + 2) * 0.5) / (double) (x3[i] + 0.7) +
                qrand() / (double) RAND_MAX * 0.01;
    }
...
void MainWindow::setupAdvancedAxesDemo (QCustomPlot *customPlot){
    ...
    wideAxisRect->addAxis(QCPAxis::atLeft)->setTickLabelColor(
            QColor("#6050F8")); // add an extra axis on the left and color its numbers
    ...
    customPlot->plotLayout()->addElement(1, 0,
                                         subLayout); // sub layout in second row (grid layout will grow accordingly)
    ...
    // create and configure plottables:
    QCPGraph *mainGraphCos = customPlot->addGraph(wideAxisRect->axis(QCPAxis::atBottom),
    ...
    QCPGraph *mainGraphGauss = customPlot->addGraph(wideAxisRect->axis(QCPAxis::atBottom),
    ...
    QCPGraph *subGraphRandom = customPlot->addGraph(subRectLeft->axis(QCPAxis::atBottom),
...
void MainWindow::setupFinancialDemo (QCustomPlot *customPlot){
    ...
    for (int i = 0; i < n / 5; ++i){
        ...
        (v < 0 ? volumeNeg : volumePos)->addData(startTime + 3600 * 5.0 * i,
                                                 qAbs(v)); // add data to either volumeNeg or volumePos, depending on sign of v
    }
    ...
    connect(customPlot->xAxis, SIGNAL(rangeChanged(QCPRange)), volumeAxisRect->axis(QCPAxis::atBottom),
            SLOT(setRange(QCPRange)));
    connect(volumeAxisRect->axis(QCPAxis::atBottom), SIGNAL(rangeChanged(QCPRange)), customPlot->xAxis,
            SLOT(setRange(QCPRange)));
...
void MainWindow::realtimeDataSlot (){
    ...
    if (key - lastFpsKey > 2) // average fps over 2 seconds
    {
        ui->statusBar->showMessage(
                QString("%1 FPS, Total Data points: %2").arg(frameCount / (key - lastFpsKey), 0, 'f', 0).arg(
                        ui->customPlot->graph(0)->data()->size() + ui->customPlot->graph(1)->data()->size()), 0);
        lastFpsKey = key;
        frameCount = 0;
    }
}
...
void MainWindow::bracketDataSlot (){
    ...
    if (key - lastFpsKey > 2) // average fps over 2 seconds
    {
        ui->statusBar->showMessage(
                QString("%1 FPS, Total Data points: %2").arg(frameCount / (key - lastFpsKey), 0, 'f', 0).arg(
                        ui->customPlot->graph(0)->data()->size()), 0);
        lastFpsKey = key;
        frameCount = 0;
    }
}
...
void MainWindow::screenShot (){
    ...
    #else
    QPixmap pm = qApp->primaryScreen()->grabWindow(qApp->desktop()->winId(), this->x() - 7, this->y() - 7,
                                                   this->frameGeometry().width() + 14,
