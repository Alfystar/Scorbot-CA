...
from py_Side.comLib import *
from uiClass.UiClass import Ui_UiClass
...
class MyUiQt(Ui_UiClass):
    """genera gli oggetti dell'interfaccia grafica"""
    ...
    def __init__(self, windows):
        ...
        self.loadStoredConf()  # funzione per il settaggio dei parametri della quarta schermata(salvati su file)
        ...
        self.Pulsante_invio_pagina_Basics.clicked.connect(self.sendEn)
        # pulsante seconda pagina
        self.PulsanteInvioPAginaAdvance.clicked.connect((self.sendDeg))
        # pulsante terza pagina
        self.PulsanteInvioPAgina_Inverse.clicked.connect((self.sendInvCin))
        # pulsante quarta pagina (set impostazioni)
        self.Home.clicked.connect(self.updateSetup)
        # pulsante quarta pagina (reset impostazioni)
        self.reset.clicked.connect(self.resetConf)

    """prende in ingresso una lista di angoli e ritorna una lista di encoder 
    tramite l' uso dei parametri alfa,beta,gamma e delta definiti in self.param"""

    def deg2en(self, lista):
        lista_e = []
    ...
    """prende in ingresso una lista di encoder e ritorna una lista di angoli tramite 
    l' uso dei parametri alfa,beta,gamma e delta definiti in self.param"""

    def en2deg(self, lista):
        lista_a = []
    ...
    """cinematica diretta. Prende in ingresso una lista di angoli espressi in gradi
     e restituisce una lista con x,y,z della pinza"""
    ...
    """cinematica inversa. Prende una lista di x,y,z e restituisce una lista di angoli espressa in gradi"""
    ...
    """
    funzione di inserimento dati nelle Qlabel della prima, 
    seconda e terza pagina (se tipo='0') o di dati nella quarta pagina(se tipo='1')
    ...
    def updateSensor(self, lista, tipo):
        ...
        if tipo == 0:  # se ho un messaggi di dati([encoder,correnti])
            ...
            for i in self.inVal:  # max=l1+l2+l3,min=-(l1+l2+l3)
                ...
                i.setMinimum(-int(self.structVal[0].value() + self.structVal[1].value() + self.structVal[
                    2].value()))  # il 20000 e' da prendere dalle impostazioni
            self.saveConf()  # salvo le nuove impostazioni su file

    """funzione pulsante prima pagina"""

    def sendEn(self):
        lista = []
        for i in range(0, len(self.encoder1)):  # prendo i valori degli encoder
            lista.append(self.encoder1[i].value())
        lista_a = self.en2deg(lista)  # li converto in angoli
    ...
    """funzione pulsante seconda pagina"""

    def sendDeg(self):
        lista = []
        for i in self.angle:  # prendo i valori degli angoli
            lista.append(i.value())
        lista_e = self.deg2en(lista)  # li converto in encoder
    ...
    """funzione pulsante terza pagina"""
    ...
    def sendInvCin(self):
        ...
        lista_e = self.deg2en(lista_a)  # li converto in encoder
        for i in range(0, len(self.encoder1)):  # aggiorno i valori nella prima pagina
            self.encoder1[i].setValue(lista_e[i])
    ...
    """funzione pulsante home quarta pagina"""
    ...
    def updateSetup(self):
        ...
        self.saveConf()  # salvo le nuove impostazioni
        invia(lista, "i")

    """funzione per salvare su file le nuove impostazioni"""
    ...
    def saveConf(self):
        ...
        with open('ConfFile/config.ini', 'w') as configfile:
            config.write(configfile)

    """setta le impostazioni della quarta pagina tramite i valori definiti nel file config_init.py"""

    def resetConf(self):
        self.Gomitoalto.setChecked(True)

        self.loadConf("ConfFile/.config_init.ini")
    ...
    """ Preso un file di configurazione, carica in memoria le 
    impostazioni salvate e imposta lo stato della clase """
    ...
    """ setta le impostazioni della quarta pagina tramite i valori definiti nel file config.py """

    def loadStoredConf(self):
        self.Gomitoalto.setChecked(True)
        self.loadConf("ConfFile/config.ini")
