...
int invia(struct messaggio *m, char *tipo) {
    ...
    if (strcmp(tipo, "e") == 0) {
        ...
        sprintf(mes, "i|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d", m->imp->maxEn[0], m->imp->maxEn[1],
                m->imp->maxEn[2], m->imp->maxEn[3], m->imp->maxEn[4], m->imp->maxEn[5], m->imp->minEn[0], m->imp->minEn[1],
                m->imp->minEn[2], m->imp->minEn[3], m->imp->minEn[4], m->imp->minEn[5], m->imp->maxCurrMed[0], m->imp->maxCurrMed[1],
                m->imp->maxCurrMed[2], m->imp->maxCurrMed[3], m->imp->maxCurrMed[4], m->imp->maxCurrMed[5]);
    }
...
void *readerPipe(void * pVoid) {//ricevi da Python
